    ...
    function getKiblat() {
      if ("geolocation" in navigator) {
        navigator.geolocation.getCurrentPosition(function(position) {
          ...
          // Menghitung arah Kiblat menggunakan rumus Haversine
          var kiblatLat = Math.atan2(Math.sin(longitude-39.8283), 
                                     Math.cos(latitude)*Math.sin(21.4172)-Math.sin(latitude)*Math.cos(21.4172)*Math.cos(longitude-39.8283));
          var kiblatLon = Math.atan2(Math.sin(latitude-21.4172), 
                                     Math.sqrt((Math.cos(latitude)*Math.sin(longitude-39.8283))*(Math.cos(latitude)*Math.sin(longitude-39.8283))
                                     +(Math.cos(21.4172)*Math.sin(latitude)-Math.sin(21.4172)*Math.cos(latitude)*Math.cos(longitude-39.8283))*(Math.cos(21.4172)*Math.sin(latitude)-Math.sin(21.4172)*Math.cos(latitude)*Math.cos(longitude-39.8283))));
          // Konversi dari radian ke derajat
          kiblatLat = kiblatLat * (180/Math.PI);
          kiblatLon = kiblatLon * (180/Math.PI);

          // Tampilkan arah Kiblat
          document.getElementById("kiblat").innerHTML = "Arah Kiblat: " + kiblatLat + "°, " + kiblatLon + "°";
        });
      } else {
        document.getElementById("kiblat").innerHTML = "Geolocation tidak didukung oleh browser Anda.";
      }
    }

    // Jalankan fungsi getKiblat saat halaman diload
    window.onload = getKiblat;
  </script>
</body></html>